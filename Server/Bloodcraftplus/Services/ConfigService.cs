...
internal static class ConfigService
{
    ...
    public static class ConfigInitialization
    {
        ...
        {
            string root = Path.Combine(BepInEx.Paths.ConfigPath, MyPluginInfo.PLUGIN_NAME);
            return
            [
            root,                                     // 0
            Path.Combine(root, "PlayerLeveling"),                   // 1
            Path.Combine(root, "Quests"),                           // 2
            Path.Combine(root, "WeaponExpertise"),                  // 3
            Path.Combine(root, "BloodLegacies"),                    // 4
            Path.Combine(root, "Professions"),                      // 5
            Path.Combine(root, "Familiars"),                        // 6
            Path.Combine(root, "Familiars", "FamiliarLeveling"),    // 7
            Path.Combine(root, "Familiars", "FamiliarUnlocks"),     // 8
            Path.Combine(root, "PlayerBools"),                      // 9
            Path.Combine(root, "Familiars", "FamiliarEquipment"),   // 10
            Path.Combine(root, "Familiars", "FamiliarBattleGroups") // 11
            ];
        });
