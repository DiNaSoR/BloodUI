...
internal static class DataService
{
    ...
    */
    ...
    // ==========================
    // VAuction (EclipsePlus UI)
    // ==========================

    public readonly record struct VAuctionListingSummary(
        string AuctionKey,
        string Category,
        string ItemName,
        int PrefabGuid,
    ...
    public readonly record struct VAuctionListingDetail(
        string AuctionKey,
        string SellerName,
        string ItemName,
        int PrefabGuid,
    ...
    public static string VAuctionCategory { get; private set; } = "all";
    public static int VAuctionPage { get; private set; } = 1;
    public static int VAuctionPageCount { get; private set; } = 1;
    public static int VAuctionTotal { get; private set; } = 0;
    public static long VAuctionServerUnix { get; private set; } = 0;
    public static readonly List<VAuctionListingSummary> VAuctionListings = [];
    public static VAuctionListingDetail? VAuctionSelectedDetail { get; private set; }

    public static void ParseAuctionPageData(string payload)
    {
        ...
        try
        {
            ...
            if (!header[0].Equals("v1", StringComparison.OrdinalIgnoreCase)) return;

            VAuctionCategory = header[1];
            VAuctionPage = int.TryParse(header[2], out int page) ? page : 1;
            VAuctionPageCount = int.TryParse(header[3], out int pc) ? pc : 1;
            VAuctionTotal = int.TryParse(header[4], out int total) ? total : 0;
            VAuctionServerUnix = long.TryParse(header[5], out long unix) ? unix : 0;

            VAuctionListings.Clear();
            ...
            for (int i = 1; i < segments.Length; i++)
            {
                ...
                VAuctionListings.Add(new VAuctionListingSummary(
                    AuctionKey: f[0],
                    Category: f[1],
                    ItemName: f[2],
                    PrefabGuid: int.TryParse(f[3], out int pg) ? pg : 0,
                    Quantity: int.TryParse(f[4], out int qty) ? qty : 0,
                    CurrentBid: int.TryParse(f[5], out int cb) ? cb : 0,
                    BuyNow: int.TryParse(f[6], out int bn) ? bn : 0,
                    CurrencyPrefabGuid: int.TryParse(f[7], out int cur) ? cur : 0,
                    ExpiresUnix: long.TryParse(f[8], out long exp) ? exp : 0,
                    Flags: int.TryParse(f[9], out int flags) ? flags : 0
                ));
            }
        }
        catch (Exception ex)
        {
            Core.Log.LogWarning($"[DataService] Failed to parse VAuction page: {ex.Message}");
        }
    }

    public static void ParseAuctionDetailData(string payload)
    {
        if (string.IsNullOrWhiteSpace(payload))
        {
            return;
        ...
        try
        {
            ...
            if (!f[0].Equals("v1", StringComparison.OrdinalIgnoreCase)) return;

            VAuctionSelectedDetail = new VAuctionListingDetail(
                AuctionKey: f[1],
                SellerName: f[2],
                ItemName: f[3],
                PrefabGuid: int.TryParse(f[4], out int pg) ? pg : 0,
                Quantity: int.TryParse(f[5], out int qty) ? qty : 0,
                StartingBid: int.TryParse(f[6], out int sb) ? sb : 0,
                CurrentBid: int.TryParse(f[7], out int cb) ? cb : 0,
                BuyNow: int.TryParse(f[8], out int bn) ? bn : 0,
                CurrencyPrefabGuid: int.TryParse(f[9], out int cur) ? cur : 0,
                ExpiresUnix: long.TryParse(f[10], out long exp) ? exp : 0,
                Status: f[11],
                MinNextBid: int.TryParse(f[12], out int mn) ? mn : 0
            );
        }
        catch (Exception ex)
        {
            Core.Log.LogWarning($"[DataService] Failed to parse VAuction detail: {ex.Message}");
        }
    }
}
